...
best_paths = []
q = collections.deque(best_end)
while len(q) > 0:
    path, score = q.pop()
    last = path[-1]
    if last[:2] == (initial_x, initial_y):
        best_paths.append(path)
        continue

    possible_prev = []
    if last[2] == '^':
        possible_prev.append((last[0], last[1] + 1, '^'))
        possible_prev.append(last[:2] + ('>',))
        possible_prev.append(last[:2] + ('v',))
        possible_prev.append(last[:2] + ('<',))
    elif last[2] == '>':
        possible_prev.append((last[0] - 1, last[1], '>'))
        possible_prev.append(last[:2] + ('v',))
        possible_prev.append(last[:2] + ('<',))
        possible_prev.append(last[:2] + ('^',))
    elif last[2] == 'v':
        possible_prev.append((last[0], last[1] - 1, 'v'))
        possible_prev.append(last[:2] + ('<',))
        possible_prev.append(last[:2] + ('^',))
        possible_prev.append(last[:2] + ('>',))
    elif last[2] == '<':
    ...
    for possible in possible_prev:
        if possible in visited and visited[possible] < score:
            diff = score - visited[possible]
            if diff in [1, 1000, 2000]:
                new_path = path + [possible]
                new_score = visited[possible]
                q.append((new_path, new_score))

best_spots = set()
for path in best_paths:
    for xyd in path:
        best_spots.add(xyd[:2])
